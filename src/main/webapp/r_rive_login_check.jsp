...
<body>
	...
	ResultSet rs = null;
	int flag = 0;
	session.setAttribute("uuname", name);
	if (buttonvalue.equals("Forget Password")) {
		response.sendRedirect("r_fogetpw.jsp");
	}
	else {
		if (name != "" && password != "") {
			try {
		...
		rs = statement.executeQuery("select username,password from rrive_user");
		while (rs.next()) {
		...
		if (flag == 0) {
			...
			response.sendRedirect("r_rive_view.jsp");
			statement.close();
			connection.close();
		}
